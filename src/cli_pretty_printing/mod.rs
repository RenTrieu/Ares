    ...
    match result.output_method {
        OutputMethod::Stdout => {
            ...
            if invis_char_percentage > INVIS_CHARS_DETECTION_PERCENTAGE {
                println!("{} of the plaintext is invisible characters, would you like to save to a file instead? (y/N)", invis_char_percentage);
                ...
                if result {
                    ...
                    if file_path.is_empty() {
                        file_path =
                            format!("{}/ares_text.txt", env::var("HOME").unwrap_or_default());
                    }
                    println!(
