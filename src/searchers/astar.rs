...
struct AStarNode {
    ...
    /// Name of the decoder to be used for this node
    ...
    prev_node: Option<Arc<AStarNode>>,
}
...
impl Clone for AStarNode {
    fn clone(&self) -> Self {
        AStarNode {
            ...
            next_decoder_name: self.next_decoder_name.clone(),
            prev_node: self.prev_node.clone(),
        }
    }
}
...
impl ThreadSafePriorityQueue {
    ...
    fn push(&self, node: Arc<AStarNode>) {
        let mut queue = self.queue.lock().unwrap();
        queue.push(node);
    }

    fn pop(&self) -> Option<Arc<AStarNode>> {
        let mut queue = self.queue.lock().unwrap();
        queue.pop()
    }

    fn peek(&self) -> Option<Arc<AStarNode>> {
    ...
    fn extract_batch(&self, batch_size: usize) -> Vec<Arc<AStarNode>> {
        let mut queue = self.queue.lock().unwrap();
...
// fn create_new_nodes_from_results(text: String, path: Vec<CrackResult>, cost: u32, decoders_used: Vec<CrackResult>) -> Vec<AStarNode<'a>> {
...
fn expand_node<'a>(
    ...
    for d_name in DECODER_MAP.keys() {
        // TODO: Optimize this later so we aren't cloning every iteration
        let cur_text = current_node.state.text.clone();
        let cur_path = current_node.state.path.clone();
        let heuristic = generate_heuristic(&cur_text[0], &cur_path, &None);
        neighbor_nodes.push(
            AStarNode {
                state: DecoderResult {
                    text: cur_text,
                ...
                decoder: Some(String::from(*d_name)),
                result: None,
                cost: current_node.cost + 1,
                heuristic,
                total_cost: current_node.cost as f32 + heuristic,
                cur_depth: current_node.cur_depth + 1,
...
pub fn astar(input: String, result_sender: Sender<Option<DecoderResult>>, stop: Arc<AtomicBool>) {
    ...
    open_set.push(Arc::new(AStarNode {
        state: DecoderResult {
            text: vec![input],
            path: vec![],
        },
    ...
    while !open_set.is_empty() {
        ...
        trace!(
            "Open set loop. Cur depth: {}, Open Set Size: {}",
            cur_node.cur_depth,
            open_set.len()
        );

        /* Cracking using current node */
        let result;
        match cur_node.decoder.clone() {
            Some(d_name) => {
                trace!("Cracking current node: {} at depth {}", cur_node.decoder.clone().unwrap(), cur_node.cur_depth);
                // Cracking current node
                let decoder = DECODER_MAP
                    .get(&d_name.as_str())
                    .expect("Could not obtain decoder from DECODER_MAP")
                    .get::<DecoderType>();
                result = decoder.crack(
                    cur_node.state.text.last().unwrap(),
                    &checker
                );
            ...
            // Base Case: Expand node and then pop 
            None => {
                trace!("Expanding base case at depth: {}", cur_node.cur_depth);
                let neighbor_nodes;
                if cur_node.cur_depth + 1 < config.max_depth {
                    neighbor_nodes = expand_node(
                        Arc::new(cur_node.clone()),
        ...
        // Success case
        if result.success {
            trace!("Sending successful result with {} decoders", cur_node.cur_depth);
            cli_pretty_printing::success(&format!(
                "Sending successful result with {} decoders",
                cur_node.cur_depth 
            ));
            // Reconstructing search path
            let mut path_str = String::from(cur_node.decoder.clone().unwrap());
            let mut node = Arc::new(cur_node.clone());

            trace!("Reconstructing decode path.");
            while node.prev_node.is_some() {
                match node.decoder.clone() {
                    Some(decoder_name) => {
                        path_str.push_str(" -> ");
                        path_str.insert_str(0, decoder_name.as_str());
                    },
                    None => ()
                }
                node = node.prev_node.clone().unwrap();
            }
            trace!("Reconstructed decode path: {}", path_str.clone());
            wait_athena_storage::add_plaintext_result(
                ...
                result.checker_name.to_string(),
                result.decoder.to_string(),
            );
            result_sender
                .send(Some(cur_node.state.clone()))
    ...
    result_sender
        ...
        // TODO: Fix parallel batch processing
        //
